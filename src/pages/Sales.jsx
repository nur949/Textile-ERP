...
const Sales = () => {
  ...
  const updateStatus = async (id, status) => {
    try {
      ...
      console.error(err)
      notify('Failed to update sale status', 'error')
    }
  }
  ...
  const applyBulkStatus = async (status) => {
    ...
    try {
      ...
      setSelected([])
      notify('Bulk status changes applied', 'success')
    } catch (err) {
      console.error(err)
      notify('Failed to apply bulk status updates', 'error')
    }
  }
  ...
  const downloadCsv = () => {
    ...
    notify('CSV export completed successfully', 'success')
  }
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      console.error(err)
      notify('Unable to export PDF. Please try again.', 'error')
    } finally {
      setExporting(false)
    }
  }
